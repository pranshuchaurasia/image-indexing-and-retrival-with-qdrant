...
colqwen_processor_path = os.getenv('colqwen_processor')
colqwen_model_path = os.getenv('colqwen_model')
collection_name = os.getenv('collection_name_colqwen', 'colqwen_annual_report')
...
class ColQwenIndexer:
    """
    ColQwen Document Indexer class for processing and indexing document images.
    
    Uses ColQwen2 model for multi-vector embedding generation which enables
    fine-grained visual document retrieval with late interaction scoring.
    """
    
    def __init__(self, collection_name=collection_name, 
                 model_path=None, processor_path=None):
        """
        Initialize the ColQwen indexer.
        
        Args:
            collection_name (str): Name of the Qdrant collection
            model_path (str): Path to ColQwen model (overrides env var)
            processor_path (str): Path to ColQwen processor (overrides env var)
        """
        self.collection_name = collection_name
        
        # Use provided paths or fall back to environment variables
        self.model_path = model_path or colqwen_model_path
        self.processor_path = processor_path or colqwen_processor_path
        
        if not self.model_path or not self.processor_path:
            raise ValueError(
                "Model paths not configured. Set colqwen_model and colqwen_processor "
        ...
        logger.info(f"Loading ColQwen2 model from: {self.model_path}")
        ...
        # Initialize ColQwen2 processor
        logger.info(f"Loading ColQwen2 processor from: {self.processor_path}")
        ...
        self.vector_size = 128  # ColQwen embedding dimension
...
if __name__ == "__main__":
    ...
    indexer = ColQwenIndexer(collection_name=collection_name)
